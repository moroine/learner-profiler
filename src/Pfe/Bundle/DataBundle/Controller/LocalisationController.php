...
use Pfe\Bundle\DataBundle\Entity\Localisation;
...
class LocalisationController extends Controller {
    ...
    public function renderLocalisationsAction(array $localisations, $_format) {
        ...
        $content = $engine->render('PfeDataBundle:Localisation:localisations.' . $_format . '.twig', array('localisations' => $localisations));
    ...
    public function renderLocalisationAction(Localisation $localisation, $_format) {
        $engine = $this->container->get('templating');
        $content = $engine->render('PfeDataBundle:Localisation:localisation.' . $_format . '.twig', array('localisation' => $localisation));
    ...
    public function localisationsAction($_format) {
        $repository = $this->getDoctrine()->getRepository("PfeDataBundle:Localisation");
     ...
     * @Method({"GET"})
     * @ParamConverter("localisation", class="PfeDataBundle:Localisation")
     * @Template()
     */
    public function localisationParticipantsAction(Localisation $localisation, $_format) {
        /* @var Pfe\Bundle\DataBundle\Entity\ParticipantRepository $repository */
        $repository = $this->getDoctrine()->getRepository("PfeDataBundle:Participant");

        $participants = $repository->findByHome($localisation);

        return $this->forward("PfeDataBundle:Participant:renderParticipants", array(
